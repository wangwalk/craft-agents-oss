...
export async function handleSetSessionStatus(
  ...
  try {
    ...
    if (ctx.resolveStatus) {
      ...
      // (e.g. set "needs-review"), but it must never move a session into a closed
      // state on its own — that decision belongs to the user.
      if (category === 'closed') {
        return errorResponse(
          `Refusing to set the closed status "${resolved}". Closing a task (done/cancelled) is the user's decision — leave it for them to do in the session list. If the work is ready for review, set an open status such as "needs-review" instead.`
        );
      }
      status = resolved;
    }
