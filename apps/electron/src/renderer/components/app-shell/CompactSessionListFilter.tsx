 ...
 * - Label rows toggle include; tapping the trailing mode chip on an active row
 *   toggles include ↔ exclude.
 * - Pinned filters (from the route, e.g. flagged / label views) are shown as
 *   disabled rows with a check mark, matching the dropdown.
 * - The search input filters labels using the same scoring helper as the desktop dropdown.
 */
          ...
          {!isSearching && !isStateSubView && (
            <Section title={t('sidebar.group')} icon={<Layers className="h-3.5 w-3.5" />}>
              ...
              <FilterRow
                ...
                label={t('sidebar.groupByDirectory')}
                radioSelected={chatGroupingMode === 'directory'}
                onTap={() => setChatGroupingMode('directory')}
              />
              <FilterRow
