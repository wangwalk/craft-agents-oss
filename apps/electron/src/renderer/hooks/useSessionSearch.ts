...
export interface UseSessionSearchOptions {
  ...
  groupingMode?: 'date' | 'directory' | 'status' | 'unread' | 'project'
...
function getCollapseGroupKey(item: SessionMeta, groupingMode?: 'date' | 'directory' | 'status' | 'unread' | 'project'): string {
  if (groupingMode === 'directory') return `directory-${encodeURIComponent(item.workingDirectory || '__none__')}`
